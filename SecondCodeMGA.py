...
Original file is located at
    https://colab.research.google.com/drive/1wndXLdcHhBYyH_jdqD2GZFKTrhyJoRqB
"""
...
coorCiudades=np.loadtxt('CoordenadasCiudades.txt')
...
def MutarCromosomas(listaCromosomas, excepcion=-1):
    ...
    '''
    for indexcrom in range(0,len(listaCromosomas)):
...
def CompararCiudades (ciudad,coorCiudades):
  listaDistancias=[]
  for i in range(len(coorCiudades)):
    if i != ciudad:
      a=coorCiudades[i][0]
      b=coorCiudades[ciudad][0]
      c=coorCiudades[i][1]
      d=coorCiudades[ciudad][1]
      distancia=((a-b)**2+(c-d)**2)**(1/2)
    if i == ciudad:
      distancia=1000000 #Valor mayor que cualquier distancia entre dos ciudades
    listaDistancias.append(distancia)
  siguienteCiudad = np.where(listaDistancias == np.amin(listaDistancias))
  return siguienteCiudad[0][0]

def inicializacionModificada (cantidadCromosomas, cantidadCiudades,coorCiudades):
    '''
    Crea una lista de Cromosomas 
    ...
    listaCromosomas=[]
    for icromos in range(0, cantidadCromosomas):
        coordenadas=np.array(coorCiudades)
        cromosoma=[]
        nextGen=np.random.randint(0, cantidadCiudades)
        cromosoma.append(nextGen)
        for igen in range(1, cantidadCiudades):
            nextGen=CompararCiudades (cromosoma[igen-1],coordenadas)
            cromosoma.append(nextGen)
            coordenadas[cromosoma[igen-1]]=[1000, 1000]
        '''    
        Esto no es del todo necesario, es para que no se repitan los cromosomas
        '''
        while cromosoma in listaCromosomas:
...
listaCromosomas=inicializacionModificada (cantidadCromosomas, cantidadCiudades,coorCiudades)
#print(listaCromosomas)
coorCiudades=np.loadtxt('CoordenadasCiudades.txt')
#print(coorCiudades)
Iteraciones=[]
valoresMinimos=[]
...
for i in range(cantidadIteraciones):
    ...
    nMutaciones=np.random.randint(3,10)
    for i in range(nMutaciones):
